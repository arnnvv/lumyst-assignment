...
const analysisData = analysisJson as {
  analysisData: {
...
export function convertDataToGraphNodesAndEdges(): {
  ...
  const graphNodes: GraphNode[] = analysisData.analysisData.graphNodes.map(
    (node) => ({
      id: node.id,
      label: node.label,
    }),
  ...
  const graphEdges: GraphEdge[] = analysisData.analysisData.graphEdges.map(
    (edge) => ({
      id: edge.id,
      source: edge.source,
      target: edge.target,
  ...
  const c1Output: C1Output[] = analysisData.analysisData.c1Output.map(
    (output) => ({
      id: output.id,
  ...
  const c2Subcategories: C2Subcategory[] =
    analysisData.analysisData.c2Subcategories.map((subcategory) => ({
      id: subcategory.id,
  ...
  const c2Relationships: C2Relationship[] =
    analysisData.analysisData.c2Relationships.map((relationship) => ({
      id: relationship.id,
